# pip install numpy torch torchvision timm matplotlib scikit-learn tqdm h5py
...
def load_preprocessed_data():
    ...
    with h5py.File("data/preprocessed_data.h5", "r") as f:
        X_train = torch.tensor(f["X_train"][:], dtype=torch.float32)
        X_test = torch.tensor(f["X_test"][:], dtype=torch.float32)
...
plt.subplot(1, 2, 1)
plt.imshow(X_train[0].squeeze(), cmap='viridis')
